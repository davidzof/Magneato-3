...
public class PageViewTest {
    ...
    private void addKeys(String path, JsonNode jsonNode, boolean cloneable) {
        if (jsonNode.isObject()) {
            ...
            while (iter.hasNext()) {
                ...
                if (cloneable == true || key.endsWith("_c")) {
                    addKeys(path + "/" + entry.getKey(), entry.getValue(), true);
                } else {
                    addKeys(path + "/" + entry.getKey(), entry.getValue(), false);
                }
            }
            ...
            if (cloneable) {
                System.out.println(">" + path + " : " + valueNode.asText());
            }
