...
public class PageResource {
    ...
    /**
     * Create a new "page" asset. If admin Metadata can be edited
     * Must be an Editor to create new pages
     *
    ...
    public String save(String body, @Context HttpServletRequest request,
        ...
        try {
            ...
            if (!security.isUserInRole("ADMIN")) {
                ...
                String editTemplate = jsonNode.get("metadata").get("edit_template").asText();
                metaData.setEditTemplate(editTemplate).setViewTemplate("").setIPAddr(request.getRemoteAddr()).setOwner(security.getUserPrincipal().getName());

                ((ObjectNode) jsonNode).set("metadata", objectMapper.readTree(metaData.toJson()));
