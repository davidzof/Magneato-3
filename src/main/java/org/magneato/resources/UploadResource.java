...
public class UploadResource {
	...
	public View uploadGPX(@FormDataParam("file") final FormDataBodyPart body,
			@FormDataParam("file") final InputStream fileInputStream,
			@Context HttpServletRequest request,
			@Context SecurityContext security) {
		return uploadGPXClone("", body, fileInputStream, request, security);
	}
	...
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	@Produces(MediaType.TEXT_HTML)
	public View uploadGPXClone(@PathParam("parent") String parent,
		...
		// TODO move to config
		String template = "{\"title\":\"%1$s\",\"child\":false,\"activity_c\":\"\",\"trip_date\":\"%11$s\",\"difficulty_c\":{\"rating\":\"\"},\"ski_difficulty_c\":{\"rating\":\"\"},\"technical_c\":{\"orientation\":\"\",\"distance\":%2$.3f,\"climb\":%3$d,\"descent\":%4$d,\"min\":%5$d,\"max\":%6$d,\"location\":{\"lat\":%7$s,\"lon\":%8$s}},%9$s,\"metadata\":%10$s}";
		String content = null;
			...
			if (parent != null && !parent.isEmpty()) {
				...
				try {
					...
					for (Map.Entry<String, String> entry : clonableItems
							.entrySet()) {
						setTokenValueIfEmpty(contentTree, entry.getKey(),
								entry.getValue());
					}//
	...
	public View uploadView(
			@DefaultValue("true") @QueryParam("clone") boolean clone,
			@Context HttpServletRequest request) {
		String id = "";
		if (clone) {
			id = pageUtils.getId(request.getHeader("referer"));
		}
	...
	private void setTokenValueIfEmpty(JsonNode root, String path, String value) {
		...
		for (int i = 1; i < keys.length; i++) {
			...
			if (nextToken == null) {
				...
				if (i + 1 == keys.length) {
					// check to see if nextToken is empty
					if (nextToken.asText().isEmpty()) {
						break;
					}
					return;
				} else {
