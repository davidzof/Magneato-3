...
/**
 * A {@link HealthCheck} which checks the cluster state of an Elasticsearch
 * cluster.
 * 
 * @see <a
 *      href="http://www.elasticsearch.org/guide/reference/api/admin-cluster-health/">Admin
 *      Cluster Health</a>
 */
public class EsClusterHealthCheck extends HealthCheck {
	 ...
	 *            cluster
	 * @param failOnYellow
	 *            whether the health check should fail if the cluster health
	 *            state is yellow
	 */
	...
	/**
	 * Construct a new Elasticsearch cluster health check which will fail if the
	 * cluster health state is {@link ClusterHealthStatus#RED}.
	 * 
	 * @param client
	 *            an Elasticsearch {@link Client} instance connected to the
	 *            cluster
	 ...
	 * 
	 * @return if the Elasticsearch cluster is healthy, a healthy
